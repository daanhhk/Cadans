...
**STAP 7 BOUWVOLGORDE 1 AF — DE TSS-WEGING GEIJKT EN LIVE (juli 2026).** Engine-commit `0f95f25` (vijf bestanden: `zones.ts`, `archetypes.ts`, `planner.ts`, `selftest.test.ts` plus de herijkte `apps/web/src/lib/onderhoudInvariance.test.ts`), CI success (run <https://github.com/daanhhk/Cadans/actions/runs/30271370595>), plus deze close-out. **Prod NU Worker Version `3fef6cd0-a67a-436e-b8fd-1162a9ea2b1f`** (was `6bd05cd2-6aea-4b71-95de-81a226e74dd4`). Die deploy bracht `e6b3e4a` (bouwitem 2 stap 1 en 3) ÉN `0f95f25` samen live — beide stonden ongedeployed op main. 3 assets vervangen, 63 ongewijzigd. GEEN migratie en geen enkel wrangler-d1-commando; `0006` blijft de laatste remote.
- **VLOEREN NU: vitest-totaal 686 · engine-selftest-assert-count 1286** (van 683/1260). Lees ze uit de suite; hardcode ze nooit.
- **WAT ER STAAT.** `zones.ts` draagt twee geëxporteerde tarieftabellen met een herkomstnoot: `ZONE_TSS_RATE_` (rust 0,60 · z2 0,73 · tempo 1,14 · drempel 1,35 · anaeroob 3,08) en `LOAD_TSS_RATE_` (low 0,695 · high 1,208 · anaerobic 3,045). Nieuwe pure functie `tssFromBlokken_` sommeert minuten × tarief over de VIJF `pctZoneBucket_`-zones en rondt ÉÉN keer aan het eind; onbekende of ontbrekende zone valt terug op het z2-tarief. Vier aanroepplekken om: `expandArchetype_`, `renderVariant_`, `buildFreeRideWorkout_`, `genericLongZ2`. `tssFromZoneMinutes_` houdt zijn drie buckets, nu geijkt, en bedient nog uitsluitend `genericPendelIntervals`.
- **DE ONTWERPPREMISSE IS GEMETEN, NIET AANGENOMEN.** `blokken` teruggevouwen via `ARCHETYPE_LOAD_FROM_BUCKET_` is gelijk aan `intent`, en de blokminuten tellen op tot `totaalMin`. Chat-zijde onafhankelijk gemeten over 700 archetype- en 2250 variant-renderingen (nul afwijkingen), CC-zijde over 1020 renderingen op duurRange-grenzen (nul afwijkingen, elke rendering draagt blokken). Daarom kon TSS naar vijf buckets ZONDER `intent` aan te raken: de dosis-valuta uit `DOELEN-SPEC` §2A is ongemoeid, en `blok.ts` leest sowieso geen TSS.
- **DE GEMETEN GRONDSLAG.** Kleinste kwadraten door de oorsprong op de kruisproducten in `docs/STAP7-IJKING-DATA.md`, chat-zijde onafhankelijk nagerekend: 0,5975 · 0,7335 · 1,1419 · 1,3505 · 3,0765, R² 0,9906, RMSE 8,264; drie buckets 0,6952 · 1,2078 · 3,0452 met RMSE 8,422. De gedeployde weging stond 19,3% te laag. De vijf grenzen vallen samen met de zones waarop intervals.icu meet — `tryPowerZoneTimes_` vouwt Z1..Z7 met dezelfde mapping.
- **GEMETEN EFFECT.** Weken +4% tot +12%: FTP/Base/mesoweek 1 ma60 di60 do60 za120 van 226 naar 253 · di60 do60 za120 zo60 van 221 naar 235 · Onderhoud/Base ma45 wo45 vr45 za90 van 172 naar 186. Losse sessies: drempeluur +13%, VO2-uur +37%, rustige duurrit ONVERANDERD. De archetype-bibliotheek op 60 minuten +30%. Minder dan de 19,3% op de echte reeks, en dat is verklaarbaar: het plan draagt 26% rust-minuten tegen 19% in de reeks, en `rust` ging van 0,70 naar 0,60.
- **48 VAN DE 48 VINGERAFDRUKKEN GEWIJZIGD, UITSLUITEND IN TSS** — naam, type, duur en zones byte-identiek, in code bewezen vóór het overschrijven. Dat is het sterkste signaal uit deze ronde: de karakter-invariantie hield, alleen de meter bewoog. LET OP: die test is daarmee opnieuw geijkt tegen de gewijzigde engine en bewijst voor DEZE wijziging niets. Het bewijs zijn de directe tarief-tests in `selftest.test.ts`.
- **CORRECTIE OP HET CC-RAPPORT — de blokminuten-notitie klopt niet.** Gerapporteerd was: 13 renderings, uitsluitend `vo2_3030` en `vo2_4020`, "nooit méér". Chat-zijde nagemeten: 104 renderings over 21 varianten, en de afwijking gaat BEIDE kanten op — in 63 gevallen liggen de blokminuten bóven `totaalMin`. De grens van ±0,96 minuut klopt wél, dus de tolerantie van 1,0 houdt en de test is goed. Het is een afrondingsartefact van de mesoFactor-werkschaling: het vuurt alleen in mesoweek 2 en 3, nooit in week 1 waar de factor 1 is. Bovengrens van de gevolgen 0,7 TSS op een sessie. HET TESTCOMMENTAAR IS FOUT OP OMVANG ÉN RICHTING — rechtzetten bij de volgende engine-aanraking.
- **CC-CORRECTIE OP DE PROMPT, terecht.** `renderVariant_` leest de omgevingsklok NIET; `weekIndexFromStart_` zit in `buildWorkout` via `selectVariant_`. De klokpinning die de prompt voorschreef was overbodig. CC pinde in plaats daarvan `doelStart` zodat de test deterministisch is by construction.
- **DE PENDEL LEEK ALARMEREND EN IS HET NIET.** Het drievoudige anaerobe tarief tilt een VO2max-pendel van een uur naar 75 TSS, méér dan een échte VO2max-sessie (63) — maar gemeten over alle fases is `pendel_intervals` ALLEEN bereikbaar in een Test-week. In Base, Build en Peak claimt de endurance-fill de pendeldag als `pendel_z2`, in een deload idem, in event-herstel wordt het `recovery`. Het getal is vrijwel dood; niet escaleren, wel meenemen wanneer de pendel-bouwer toch open ligt.
- **DE TWEEDE TSS-FAMILIE IS DE VOLGENDE STAP, in gemeten volgorde van bereikbaarheid.** Ongeveer 35 workouts dragen een VLAK tarief (`Math.round(mins × c)`), zonder blokken en zonder intent, en zijn dus NIET geijkt. Live in een normale week: NIETS — familie A draagt daar 100% van de TSS. Wél live: (1) `recovery` op 0,35/min tegen circa 0,60 geijkt, elke deloadweek, gemeten 42 van 172 week-TSS · (2) `pendel_z2` op 0,60/min terwijl een pendel grotendeels Z2 is (0,73), elke pendeldag in Base/Build/Peak — sluit meteen de bekende bijvangst dat de dosis-valuta op een pendeldag nul telt · (3) `combo_long_with_efforts`, draagt géén blokken en zet zijn tss los van zijn eigen intent (153 tegen 130), reist mee met bouwitem 2 stap 2 en 4 · (4) `pendel_*_intervals`, alleen Test-week · (5) taper, `test` en de legacy `vo2_*`-familie: slapend of niet meer geselecteerd.
- **OPENSTAAND — VISUELE CHECK door Daan.** Hard refresh of incognito i.v.m. SW-cache. Verwacht: de gepland-kant van de weekkaart staat 6 tot 12% hoger en het verschil zit in de KWALITEITSDAGEN, niet in de duurritten.
- **OPENSTAAND — GEMENGDE WEGING, één overgangsweek.** `workoutFromFrozenEntry` (`apps/web/src/lib/proposal.ts`) leest bewaarde TSS VERBATIM, zonder herberekening, dus verstreken dagen houden de oude weging. Gedeployd op maandag 27-07, dus de vervuiling is praktisch nul. De weekplan-blob bewaart `blokken` op ENTRY-niveau (niet per sessie), dus een herberekening achteraf is technisch mogelijk maar niet gedaan.
...
- **OPENSTAAND — DE CTL-SIMULATIE MOET OPNIEUW.** `STAP7-RECON` §6 leunde op de oude weging en leverde de onhoudbare "negen uur per week"-conclusie. Nu de weging geijkt is, kan die opnieuw — met een ONAFHANKELIJK gemeten eindwaarde als ijkpunt (`WERKWIJZE.md`).
- **OPENSTAAND — STAP 2 EN 4 REIZEN SAMEN (ongewijzigd).** Het duur-plafond in de kandidaat-filter van `goalWorkout_` staat er BEWUST nog; eraf zonder selectieregel laat de tie-break het kortste sjabloon winnen. De selectieregel komt uit de COACH-CANON, niet uit de D1-meting; die levert alleen een bovengrens-check.
...
- **FOCUS VOLGENDE CHAT: de tweede TSS-familie (bouwvolgorde 1, restant).** `recovery` en `pendel_z2` eerst — die zijn elke week live. Recon-first: beide bouwers dragen geen blokken en geen intent, dus de vraag is of ze een blokken-structuur krijgen (en daarmee ook de dosis-valuta vullen) of een geijkt vlak tarief. ENGINE, expliciete autorisatie, selftest-vloer stijgt mee. Neem het foute testcommentaar over de blokminuten-speling mee. Verse chat.
