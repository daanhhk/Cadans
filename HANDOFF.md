...
- **OPENSTAAND, HOOGSTE PRIORITEIT — DE INGESTELDE FTP KLOPT NIET MEER.** In alle 142 rijen wijkt `rolling_ftp` af van de `ftp`-kolom; nu 262 gemeten tegen 280 ingesteld, grootste afwijking −18 op 25-07. Gevolg: de zonegrenzen staan te hoog, dus TSS en daarmee CTL worden te LAAG berekend — dat raakt de doortrain-kaart en de blok-check die nu juist op een dalende CTL oordelen. Er staat een `ftpAutoUpdate`-vlag (bool 0/1) in `workers/api/src/db/schema.ts`; NIET onderzocht of die uit staat of niet draait. OPEN bevinding, geen besluit: eerst vaststellen welke waarde de waarheid is, want een FTP-wijziging herschrijft met terugwerkende kracht elke zone-meting.
...
- **OPENSTAAND (geërfd, niet geraakt).** Deploy van 5a/5a-ii plus visuele check · UP-fixture in `Preview.tsx` realistischer maken · weken-terug-scrollen in de Schema-tab · gat-dag-types via meegegeven datum (ENGINE) · `docs/DOELEN-SPEC.md` §6 stap 3 doel-lijst herzien (ENGINE).
