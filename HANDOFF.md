...
- **DE VULLING BESTAAT AL — het is een SELECTIEPOORT, geen ontbrekende functie.** `expandArchetype_` met een `doelMin` ver boven `duurRange[1]` levert een correcte sessie: kwaliteitsblokken ongewijzigd, Z2-fill ertussen, totaal exact op `doelMin`, geen `tooLong`. Zonder plafond kwalificeren 35 van 35. LET OP de bekende regressie: de tie-break sorteert OPLOPEND op `duurRange[0]`, dus met het plafond eraf wint het KORTSTE sjabloon en krijgt een dag van vier uur 24 minuten drempel. De tie-break moet dus mee om: het sjabloon met de meeste werktijd dat nog past, rest Z2.
- **NORM VASTGELEGD IN `docs/DOELEN-SPEC.md` §2A (Daan-besluit).** De coach vult de beschikbaar gestelde tijd optimaal in; DUUR IS EEN EIGENSCHAP VAN DE DAG, GEEN VOORSCHRIFT VOOR DE INHOUD. Het enige vaste element is de heenrit van de pendel. ER BESTAAT GEEN BESCHERMDE LANGE RIT. Die norm was meermaals in chats gegeven en nooit vastgelegd; nu is het een `git diff`, geen gevoel.
...
- **FOCUS VOLGENDE CHAT: de D1-IJKING** (read-only SELECT op `activities`: `tss`, `zone_times_json`, `duur_min` — de mapping zonetijd→TSS fitten op de echte ritten met de gemeten TSS als grondwaarheid), daarna bouwitem 2 in de volgorde van `docs/STAP7-BOUW12-RECON.md` §9: pre-claim eruit, duur-plafond eruit, tie-break om, `demote_recent_hard` beperken tot de cross-week bescherming. ENGINE, dus expliciete autorisatie; selftest-vloer stijgt mee. Verse chat.
