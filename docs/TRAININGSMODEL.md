...
> **DE OMREKENFACTOR, besloten 24-08-2026.** M92 zegt dat de ijkinspanning de drempelwaarde INSTELT, maar niet met welk getal. Dat gat is gemeten in `docs/PUNT69-BOUW.md` §3 en hier gesloten: **de nieuwe drempelwaarde is 95 procent van het beste twintigminutenvermogen uit de testrit**, afgelezen op het duurpunt `secs = 1200` van de per-rit-power-curve. HERKOMST-ETIKET: **Daan-besluit**, genomen op de keuze uit `docs/PUNT69-BOUW.md` §10 weg A. Het getal zelf is niet nieuw — het staat als eindopmerking op het testprotocol (`packages/engine/src/workouts/ftp.ts:319`, *"Nieuwe FTP = 95% van gemiddeld vermogen over de 20 min"*) en tweemaal in de bevroren GAS-bron — maar dat gaf het geen gezag: `docs/WERKWIJZE.md:57` stelt dat GAS een PORT-referentie is en geen normbron. Dit besluit geeft het dat gezag wel.
> DE AANVAARDE AFWIJKING, en zij is expliciet aanvaard en niet over het hoofd gezien: de kromme levert de beste twintig minuten van de HELE rit, terwijl de tekst op het protocol slaat op het gemiddelde over het blok `20-MIN ALL-OUT`. Die twee lopen uiteen zodra het beste venster het testblok niet exact dekt, en het maximum over alle vensters is per constructie ≥ dat blokgemiddelde. De uitkomst valt dus eerder iets te hoog uit dan te laag. Aanvaard omdat het alternatief een tweede verzoek per rit kost én ervan uitgaat dat de renner de structuur exact rijdt.
> DRIE RANDVOORWAARDEN HOREN ONLOSMAKELIJK BIJ DIT BESLUIT. (1) Het voorstel vuurt ALLEEN bij doel FTP — de app biedt per doel een ander testprotocol aan en alleen dat ene draagt een omrekening (`packages/engine/src/planner.ts:2071`). (2) Er hoort een plausibiliteitsgrens bij, want de kromme geeft ook voor een rustige duurrit een keurig getal: gemeten geval `0,95 × 195 = 185` W tegen een gezette FTP van 280, een verlaging van 34 procent na een Z2-rit. Zonder die grens zegt de app "gemeten" waar niets gemeten is, en dat is M5. (3) Pas bij GOEDKEURING is er geijkt; het voorstel alleen ijkt niets (M91).
