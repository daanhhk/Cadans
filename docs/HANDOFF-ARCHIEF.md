# Cadans — HANDOFF-ARCHIEF

De oudere STAND-blokken plus de historische projectsecties, verbatim uit `HANDOFF.md` gehaald op
08-08-2026. Er is niets weggegooid en niets herschreven; git houdt bovendien alles.

De reden is gemeten: de opener-fetch kapt af rond 121 kB, en `HANDOFF.md` stond op 616512 bytes — een
chat kreeg er 121124 van binnen, 19,6 procent, en zag dus alles vanaf ongeveer het vierentwintigste
STAND-blok niet. De opener haalt DIT bestand bewust niet op; wie hier iets zoekt grept het van schijf
of uit de publieke repo.

De levende projectstand staat in `HANDOFF.md`, met de twaalf nieuwste STAND-blokken.
...
**PUNT 11 IS GEMETEN EN GEPARKEERD, NIET GEBOUWD (5 augustus 2026).** Docs-only ronde. Meetcommit `76f674701f4ff27ac0aa93fe6b47c27a3c0faabe` met `docs/PUNT11-MEETDATA.md`, plus deze close-out. GEEN code, geen engine, geen migratie, geen deploy, en geen `wrangler`-commando behalve de read-only `SELECT` van de meetronde — die gaf `rows_written: 0` en `changed_db: false`.
- **HET INSTRUMENT IS EERST GEIJKT:** eigen integratie van de watts-stream tegen `icu_joules` op de drie langste ritten, 0,00% · 0,00% · −0,00%. **Die ijking was NIET voldoende** — zie de as-vondst hieronder.
- **DE UITSLAG IS B UIT HET CRITERIUM-BLOK**, en dat criterium stond gepind VÓÓR de meting. De getallen: noemer **183** fietsritten vanaf 2025-09-01, **25** kandidaten, **14** met een 20-minutenvenster ná de drempel. `t_ster` ligt over alle 25 tussen **90 en 125** minuten met mediaan **99**. De verhouding `p20_na` gedeeld door `p20_voor` correleert **+0,70** met `rest_min`; mediaan **0,830** bij een resttijd onder 60 minuten tegen **1,095** daarboven; en het beste 20-minutenblok van de HELE rit valt ná de drempel in **4 van de 7** lange ritten en in **0 van de 7** kortere. De maat meet dus de RITKEUZE, niet het duurvermogen — de richting is omgekeerd aan wat de maat hoort te betekenen.
- **DE CHAT-ZIJDIGE BOVENGRENS HIELD.** Vooraf gaf een bovengrens-schatting op de gecommitte dump 17 van 177; gemeten zijn het er 14 van 183. Een bovengrens die BOVEN de uitkomst blijft is het bewijs dat de schatting eerlijk was — was hij eronder gebleven, dan had de schatting de meting gestuurd.
- **DE AS-VONDST, CC-afwijking en de belangrijkste van de ronde.** De `time`-stream van intervals.icu is VERSTREKEN tijd met dt-sprongen op de pauzes; het AANTAL SAMPLES is de rijtijd. Op de klok-as gaf 2026-07-18 een `t_ster` van **152 minuten op een rit van 127** — een onmogelijkheid. De arbeid-ijking gaf op BÉÉDE assen 0,00% en kon dit per constructie niet vangen; de identiteit `t_ster + rest_min = duur_min` wél, en die klopt na correctie op **25 van de 25**. Alle zeven grootheden zijn opnieuw berekend op de sample-as. Staat nu als les in `docs/WERKWIJZE.md`.
- **WAT DAAN MERKT: NIETS.** Er verandert geen letter aan de app. Dit blok legt vast WÁÁROM de maat er nu niet komt, zodat hij over een half jaar niet opnieuw wordt voorgesteld alsof de vraag nog open is.
- **PUNT 11 STAAT OP GEPARKEERD MET DATUM**, niet op open. De heropen-voorwaarde staat in ROADMAP punt 11: het actieve doel moet een klim-doel zijn ÉN de lange rit structureel boven de twee uur liggen — per `DOELEN-SPEC` §3.3 tussenstap (iii) het specificiteitsblok van maart–april 2027, vroegst **2027-02-22**. GEEN vooruit-bedrading: er komt nu geen `arbeid`-kolom en geen afgeleide zonder lezer. M39 blijft OPEN, nu met reden én datum in plaats van een open vraag.
- **OPENSTAAND, NIEUW: ROADMAP punt 34 — de effect-referent kent het doel niet.** `effect.ts` geeft op case-sensitive "doel" 0 treffers, en de call-site in `blok.ts` geeft `buildEffectReferent` geen doel mee. De meter is `rolling_ftp` voor alle vijf doelen, terwijl `DOELEN-SPEC` er voor drie een andere aanwijst — en `blokEffectRegel` bewéért dat die FTP "precies de winst" is. Bij doel FTP inert, onwaar zodra het doel naar Korte beklimmingen gaat.
- **NIEUWE WERKWIJZE-AFSPRAAK.** Een goedkeurpoort in CC wordt VOORAF in de chat aangekondigd, met het antwoord erbij: Daan beoordeelt in CC niets inhoudelijks, hij herkent alleen of een gevraagde goedkeuring aangekondigd was. En je toetst of de poort NÓDIG is — de remote-D1-lezing van deze meetronde was overbodig, want het intervals-object draagt gewicht en FTP al: 0 van de 25 ritten viel terug op `icu_weight`.
- **VLOEREN NU: vitest-totaal 958 over 75 bestanden · engine-selftest-assert-count 1648**, alle drie afgelezen uit de suite-uitvoer van DEZE ronde. Lees ze zelf uit de suite; neem ze niet over uit dit blok.
- **OPENSTAAND, ONGEWIJZIGD — elk item opnieuw gegrept.** ROADMAP 13 · 16 · 19 · 21 · 22 · 23 · 24 · 25 · 28 · 31 · 32 · 33 · 34 · `indoor_ftp` 260 tegen `ftp` 280 · de 404 op `/api/checkin/<datum>` · de coach-copy-ronde · geen harness-scenario voor fase Peak · de prod-modus-belofte van de harness uit het vorige blok. **Punt 11 is eruit.** Alle dertien genummerde punten dragen bij hergrep nog status "open". ONVERANDERD SINDS 04-08: `indoor_ftp` 260 tegen 280 en de coach-copy-ronde hebben nog stééds GEEN plek in de ROADMAP — 0 treffers — en reizen dus mee zonder bron.

FOCUS VOLGENDE CHAT: ROADMAP punt 13 — na het event volgt geen herstel. Dat punt is ENGINE plus CLIENT, dus RECON-FIRST met een stop-en-verifieer voordat er ook maar één regel engine wordt aangeraakt — een echte engine-bug wordt geflagd, nooit stilzwijgend gepatcht. Het draagt de vraag om een NIEUW DOEL na het event mee, dus die hoort in dezelfde ronde beantwoord of expliciet geparkeerd. Verse chat.

**PUNT 17 IS AF EN STAAT LIVE (5 augustus 2026).** Worker Version `734ac3b8-2191-404d-97f1-2e5955ec52da`. Commits: `18cc242` fase A (het plan als referent), `1fd047f` fase B (de getoonde minuut), bouwdoc `docs/PUNT17-BOUWDOC.md` op `6a4d3ef`, plus deze close-out. Geen migratie deze deploy.
- **WAT ER VERANDERD IS.** De uitvoerings-referent van de blok-terugblik meet niet langer tegen een DOEL-BREED richtgetal maar tegen het BEWAARDE PLAN van die week — op de per-zone-eis én op de totaal-eis — en het oordeel valt op de GETOONDE HELE MINUTEN. `blokDosisNorm` bestaat ongewijzigd voort, inclusief signatuur, als DOSIS-DOEL voor het plan en de trede-kaart; hij is alleen geen RECHTER meer.
- **DE GROND, chat-zijde gemeten over 405 blok-cellen en 1215 opbouwweken.** De oude regel las een blok waarin een KWART van drempel plus anaeroob naar tempo was verschoven in **286 van de 405 cellen als GELEVERD**, en een blok dat op **0,7 maal** het plan gereden was in **96**. De nieuwe regel leest die allebei op **0 van 405**, en een exact volgens plan gereden blok op **405 van 405**. Oorzaak van de speling was de SCHAAL van de zone-norm: `normTempo` is dosis maal 0,2821, een bibliotheek-gemiddelde, terwijl V1 in Build bij Korte beklimmingen 4,0 plan-tempo draagt tegen een normTempo van 22.
- **DE PREMISSE VAN PUNT 17 WAS WEERLEGD, en dat is de vijfde correctie in die lijn.** Niet de TOTAAL-eis bond maar de PER-ZONE-poort: bij Korte beklimmingen viel in Build 37 van de 81 weken op BEIDE eisen en 15 op de ZONES ALLEEN, en nul op het totaal alleen. De SMALLE wat-als — alleen het totaal plan-relatief — tilde het geheel van 261 naar 275 van 405 en liet dit doel op 36 van 81 staan: NUL kantelingen.
- **FASE B KWAM UIT DE SHOT, niet uit een test.** De kaart toonde `VO2max 8/8` naast een teller `0/2` en `VO2max 16/16` naast `1/2`: het scherm rondde af en het oordeel niet, dus een zone las als gehaald en telde als tekort. Nu lezen die 1/2 en 2/2 terwijl de getoonde cijfers geen haar bewogen. De drie tolerantie-constanten zijn VERVALLEN — gemeten bewoog de uitkomst over het hele bereik van 0,6 tot 60 seconde geen enkele cel, dus die vraag was leeg en de echte vraag was op welke KORREL je oordeelt.
- **DE POORT OP `telt` DRAAGT TWEE EISEN**, `poortHerkomst` gelijk aan "week" én `plan.werk > 0`, en allebei zijn ze aantoonbaar levend: draai er één weg en T5 valt. De tweede kwam op meting binnen — de poortset komt uit de LABELS en het plan uit de BANDEN, dus een entry met labels zonder banden gaf herkomst "week" bij een plan van nul en las TRIVIAAL geleverd.
- **MIGRATIESTAND: 0010 is de laatst toegepaste migratie, niet 0008.** `0009_amusing_mordo` kwam in `852a3ba` en `0010_uneven_scarlet_spider` in `6356221`, allebei bij punt 12 fase A. Op het moment van deployen gaf remote D1 **"No migrations to apply!"**, vastgesteld met `wrangler d1 migrations list cadans --remote` — een READ-ONLY commando; er is geen `apply` gedraaid.
- **VLOEREN NU: vitest-totaal 958 over 75 bestanden · engine-selftest-assert-count 1648**, alle drie afgelezen uit de suite-uitvoer. De assert-count is onbewogen: geen van beide bouwen raakte `packages/engine`.
- **PROD IS GECONTROLEERD, met één grens.** De harness draaide read-only tegen prod: alle vijftien schermen laden, de live bundel is `index-Be7S0vzB.js` en dus de verse build, en de enige fout is de bekende 404 op `/api/checkin/<datum>`. MAAR: **BLOK · TERUGBLIK staat NIET op het prod-weekscherm** — het lopende blok is nog niet afgerond, dus de gewijzigde kaart is daar NIET te toetsen. Die uitspraak is dus niet toetsbaar op prod; hij staat wel vast op de lokale shots.
- **CC-WAARNEMING, geen app-defect.** De harness deed in PROD-modus toch twee schrijf-aanroepen (`POST /api/sync/activities` en `POST /api/sync/wellness`), terwijl `CLAUDE.md` bij prod-modus "geen enkele schrijf-aanroep" belooft. Het zijn de sync-calls die de app bij ELKE pageload doet, dus dezelfde schrijfactie die Daan uitlokt door de app te openen — maar de belofte klopt niet en hoort of waargemaakt of bijgesteld te worden. Zie ROADMAP.
- **OPENSTAAND, NIEUW: ROADMAP punt 33.** De norm-vergelijking staat op DRIE plekken — `opNormPerZone` en `zoneOpNorm_` in `blok.ts` plus twee inline kleurcondities in `BlokReviewCard.tsx` — en de derde heeft GEEN vangnet: draai hem terug en de hele suite van 958 blijft groen. `apps/web/vitest.config.ts` draait op `environment: "node"`, zonder jsdom en zonder één `.test.tsx`. Dat is precies de plek waar het defect zat dat fase B repareerde.
- **PUNT 31 IS AANGESCHERPT: de besmetting is NIET scenario-gebonden.** 04-08 wees `klim-weekstem` en `v7-blokweek4` aan, 05-08 wees `v7-weekstem` aan met 61 gewijzigde regels in het WEEKPLAN zelf. De toestand zwerft, dus een geërfde uitsluitingslijst dekt de vorige ronde en niet de volgende: elke sweep hoort zijn EIGEN ijkrun te dragen.
- **OPENSTAAND, ONGEWIJZIGD.** ROADMAP 11 (deels) · 13 · 16 · 19 · 21 · 22 · 23 · 24 · 25 · 28 · 31 · 32 · 33 · `indoor_ftp` 260 tegen `ftp` 280 · de 404 op `/api/checkin/<datum>` · de coach-copy-ronde · geen harness-scenario voor fase Peak. Punt 17 is eruit. Nieuw op de parkeerlijst: `KWALITEIT_MIN_PER_PRIKKEL` zonder herkomst-label, en de 3 van 960 dagcellen waar op trede 4 meer gepland staat dan opgegeven (maximaal 3,8 minuten).

FOCUS VOLGENDE CHAT: het eerstvolgende open punt uit de reeks. Verse chat.

**PUNT 17 IS GEMETEN EN AFGEBAKEND, NIET GEBOUWD (4 augustus 2026).** Docs-only ronde: de chat mat zelf via een read-only kloon plus een esbuild-bundel van de engine en de munt-laag; CC deed alleen deze commit. GEEN code, geen engine, geen migratie, geen `wrangler`-commando — ook geen read. Er is NIETS afgevinkt: punt 17 blijft open.
- **HET INSTRUMENT IS EERST GEIJKT: 16 van de 16 bekende waarden gereproduceerd** — de zeven norm-getallen uit ROADMAP punt 17 en de negen per-sjabloon werkminuten uit dit document — plus de drie intent-plafonds vo2 31, drempel 42 en sweetspot 60. Zonder die ijking meet een sweep zijn eigen aannames.
- **DE PREMISSE IS GECORRIGEERD, en dat is de VIJFDE correctie in de punt-15/17-lijn.** De norm is NIET onbereikbaar in MINUTEN. De ENVELOPPE — de beste sjablonen die de bibliotheek op die dagduren kan leveren, gemeten met de kandidaat-regel uit `goalWorkout_` zelf — geeft op Daans eigen vorm ma45 di60 do60 za120 een **96 tegen een norm van 52**, en op V1 (ma60 di60 do60 za120) een **132 tegen 78, 90 en 102**. Beperkt tot de LEIDENDE intent van het doel is het **40 en 60**, en dáár is het onder norm op elke trede, in Build én Peak.
- **WAT WEL ONBEREIKBAAR IS: de norm in het KARAKTER dat het doel voorschrijft.** De meetlat telt MINUTEN en is blind voor karakter, terwijl kort en hard werk per definitie weinig minuten kost. Op een dag van 60 heeft de vo2-pool 6 kandidaten met maximum 20 en gemiddelde 14,3 werkminuten, tegen een eis van 26 per prikkel; drempel geeft daar 34,4 en sweetspot 36. Nominale medianen over de bibliotheek: vo2 20, drempel 32, sweetspot 36.
- **DE BIBLIOTHEEK-ROUTE IS GEMETEN EN AFGEWEZEN.** Het enige vo2-sjabloon dat 31 haalt is `vo2_sandwich`, en dat doet het met 8 WERKELIJKE vo2-minuten verpakt in 20 tempo-minuten. Vo2-sjablonen bijbouwen die 26 of meer dragen kan dus alleen door de prikkel te VERDUNNEN, en dat gaat regelrecht tegen `DOELEN-SPEC` §3.3 in. Het antwoord op de vraag die dit punt openzette is daarmee: **de NORM, niet de bibliotheek**.
- **DE ASYMMETRIE VORM-TEGEN-SCHAAL.** `bibliotheekSignatuur` leidt de VORM van de norm af uit de bibliotheek (gemeten tempo 0,2821 · drempel 0,5625 · anaeroob 0,1554), terwijl de SCHAAL — `KWALITEIT_MIN_PER_PRIKKEL` in `packages/engine/src/utils.ts`, FTP 28, Onderhoud 22, de rest 26 — geen afleiding én geen herkomst-label draagt. Dat schendt de eigen regel dat elk getal PLAN, SIGNAAL of BELEID draagt.
- **WELK MECHANISME BIJT, en het is niet het voor de hand liggende.** NIET de per-zone-poort van punt 14 — die laat een vo2-week juist door. Het is de TOTAAL-eis van punt 15 fase 2 (`werkTotaal >= gevraagd`, `apps/web/src/lib/blok.ts:550`), die tempo, drempel en anaeroob OPTELT en tegen een doel-breed getal legt. Punt 17 zit exact op die naad.
- **DE SCHADE, MET DATUM.** `dosisTredeVoorstel` geeft `null` tenzij de check `geleverd_gestegen` of `geleverd_niet_gestegen` leest. **Vanaf 2027-02-22**, wanneer Korte beklimmingen het actieve doel wordt, leest het A-doel dus ELK BLOK "niet geleverd" terwijl het plan exact is uitgevoerd — en de dosis-trede kan per constructie nooit stijgen.
- **HET ANKER IN DE COACH-CANON, MET DE GRENS ERBIJ.** M63 (NORM) stelt dat het tekort het VERSCHIL is tussen wat bedoeld was en wat geleverd is, naar rato bij een half uitgevoerde sessie; M64 (NORM) stelt dat alleen een gemiste SLEUTELSESSIE een ingreep rechtvaardigt. De referent is daarmee canoniek PLAN-relatief. LET OP: beide regels zijn geschreven voor de WEEK-laag; ze op de BLOK-check toepassen is een UITBREIDING en geen citaat, en hoort als zodanig verantwoord te worden.
- **OPENSTAAND, NIEUW: ROADMAP punt 32 — de rit-beoordeling is geen pijler.** M30 geeft subjectieve terugkoppeling de status "informeert"; M31 noemt twee gebouwde en niet-aangesloten sensoren; M19 noemt de betere uitkomstmaat bij naam. Die drie regels stonden BUITEN de reeks en kwamen daardoor nooit aan de beurt.
- **NIEUWE WERKWIJZE-AFSPRAAK.** De opener haalt voortaan ook `docs/TRAININGSMODEL.md` op — vier raw-URL's in plaats van drie — dat document staat nu in de voorrangslijst als NORM-laag boven `DOELEN-SPEC`, en een besluit over de trainingslaag noemt voortaan zijn M-regel of stelt vast dat de canon daar OPEN is. **DE DRAGENDE OBSERVATIE:** de norm-laag stond bóven `DOELEN-SPEC` en werd door GEEN ENKELE opener opgehaald, dus elke chat reconstrueerde de coach-bril in plaats van hem te lezen. Daan wees erop; dat hoort de bron te doen.
- **OPENSTAAND, ONGEWIJZIGD — elk item opnieuw gegrept.** ROADMAP 11 (deels) · 13 · 16 · 17 · 19 · 21 · 22 · 23 · 24 · 25 · 28 · 31 · `indoor_ftp` 260 tegen `ftp` 280 · de 404 op `/api/checkin/<datum>` · de coach-copy-ronde · geen harness-scenario voor fase Peak. **TWEE VAN DIE ITEMS HEBBEN GEEN PLEK IN DE ROADMAP, en dat is precies de drift waarvoor deze hergrep bestaat:** `indoor_ftp` 260 tegen 280 staat alleen als RUWE MEETDATA in `docs/DOSIS-MUNT-MEETDATA.md` en op geen enkele parkeerlijst, en de coach-copy-ronde staat NERGENS behalve in deze OPENSTAAND-regels zelf. Ze reizen dus mee zonder bron. De 404 en het Peak-gat staan wél op de parkeerlijst en zijn daar bevestigd.

- **VLOEREN NU: vitest-totaal 952 over 74 bestanden · engine-selftest-assert-count 1648**, alle drie afgelezen uit de suite-uitvoer. **DAT IS EEN CORRECTIE, GEEN GROEI:** het blok hieronder noteert 942 over 73, en dat getal was op het moment van schrijven al onwaar. `apps/web/src/lib/schemaLoad.test.ts` bracht in `5502238` precies 10 tests in 1 bestand mee, en de close-out `c19c900` kwam daarná maar nam de vloer van vóór die bouw over zonder te hermeten. Deze ronde raakte geen regel code — `git status` gaf drie docs-bestanden — dus 952/74 is de ware stand van `c19c900` en van `acecf7a`. De assert-count is onbewogen. Lees de vloeren uit de suite; hardcode ze nooit, en neem ze ook niet over uit het blok erboven.

FOCUS VOLGENDE CHAT: ROADMAP punt 17 — de wat-als op de referent. AFWIJKING VAN DE NUMMERING, MET REDEN: 11, 13 en 16 staan lager, maar 17 is in de reeks aangewezen als opvolger van punt 15, term (iii) van 15 fase 3c staat er expliciet achter geparkeerd, en punt 16 (ii) draagt dezelfde norm-vraag. CLIENT verwacht, geen engine. Verse chat.

**PUNT 29 EN PUNT 30 ZIJN AF; 30 STAAT LIVE (4 augustus 2026).** Worker Version `d7356039-e58c-439d-8df4-0b1a2c2c12a4`. Geen migratie, geen remote-D1-mutatie. Commits: `b3a3686` punt 29, `c852217` het bouwdoc van punt 30, `5502238` de bouw, plus deze close-out.
- **PUNT 29 — DE HARNESS SCHIET NIET MEER BLIND.** Voor elke sweep leest hij de parameterloze GET-routes uit `workers/api/src/routes/api.ts` ZELF en stopt lokaal zodra er één 404 geeft, VÓÓR de `rmSync` — dus bij een stop blijft de vorige meting staan. GROEN: `routes: 15 expected, 15 present, 0 missing, 0 unknown`. ROOD aangetoond met `/rood-toets-29`, waarna de 95 PNG's van de groene run er nog stonden. Alleen een 404 telt als missing (Hono geeft 404 op een onbekend pad, een bestaande route met ontbrekende query geeft 400); een time-out telt als `unknown`, want een vals STOP blokkeert legitiem werk.
- **PUNT 30 — HET VERDICT IS VERSCHOVEN, en dat is de kern.** Niet per-kaart-degradatie maar HERHALEN-EN-BENOEMEN. GROND: twaalf van de vijftien rijen voeden `buildWeekProposal`, en de getoonde week gaat via `persistWeekplan` (`schema.ts:1613`) als plan-van-record de opslag in — stil degraderen levert dus geen ontbrekende kaart maar een ANDER plan-van-record, zelfde vorm als punt 26. Er bleef ook maar ÉÉN rij over die veilig te laten vallen is (`dispositions`); `powerZones` voedt een oordeel en `doelPassend` onderdrukt een kaart. WAT ER KWAM: `retryLoad` herhaalt de hele bouw — 3 pogingen, 600 en 1800 ms, **BELEID en geen ijking** — en `laadGelabeld` noemt de gevallen rij bij NAAM. Zie `docs/PUNT30-BOUWDOC.md`.
- **DE GRENS VAN HET BEGRENZINGSBEWIJS, onverbloemd.** **85 van de 93 identiek, met 8 shots UITGESLOTEN omdat de IJKING op precies die 8 faalde** — `v7-midweek`, waar run 1 en run 2 op ONGEWIJZIGDE code al uiteenliepen. Ik heb de vergelijking aanvankelijk op een derde run gelegd die wél met run 1 overeenkwam; **dat is de nulmeting op de uitkomst kiezen, en die uitslag is INGETROKKEN**. Wat overeind staat is dus: op de 85 shots die aantoonbaar deterministisch zijn bewoog niets.
- **DE FOUTTAK IS NIET GEFOTOGRAFEERD**, en dat is een grens en geen omissie: de harness kan geen route laten uitvallen. Tien tests dekken hem. HET PROPAGATIEVENSTER GAF DEZE DEPLOY GEEN MEETPUNT: alle vier de bronnen — `/schema`, `/assets/index-B_KhlYth.js`, `/api/settings` en `/api/doel-passend` — gaven 200 in de eerste ronde, herstel 0,0 s. De gelabelde fouttak is dus nog steeds alleen door tests gedekt en niet door een echt scherm.
- **DRIE METINGEN DIE DE WISSELING LOSKOPPELEN VAN DEZE BOUW.** M1: alle drie de ijk-runs stonden op de VOOR-staat, zeker uit de commandovolgorde — `git checkout HEAD~1` met de controle `retryLoad aanwezig? 0` ging eraan vooraf, en `git checkout HEAD` met `retryLoad aanwezig? 2` kwam erna. De wisseling is dus OUDER dan deze bouw. M2: van de acht `v7-midweek`-shots dragen er zeven `errors: none`; de achtste draagt alleen de bekende `http 404: /api/checkin/2026-08-05`, identiek in de VOOR-run, en die valt via de fouttolerante `getCheckin` op `null` zonder te gooien. Er is dus niets gefaald en `retryLoad` heeft NOOIT gevuurd. M3: het STAND-blok van de punt 12-ronde noemt `doel-passend` (`01-week` t/m `08-zo`), NIET `v7-midweek` — dat begrenzingsbewijs klopte en behoeft geen correctie.
- **OPENSTAAND, NIEUW: ROADMAP punt 31.** De harness besmet mogelijk zijn eigen nulmeting. KANDIDAAT, niet vastgesteld: `persistWeekplan` schrijft bij ELKE pageload naar D1, dus run 1 kan run 2 besmetten, en `v7-midweek` is met `dagOffset` 2 het gevoeligst. De uitsluitende toets staat erbij: twee opeenvolgende sweeps met de weekplan-tabel ervoor én erna gelezen. Apart van punt 23, want dat gaat over pixel-verschil bij identieke `innerText` en dit mogelijk over verschoven TOESTAND.
- **CC-WAARNEMING, geen app-defect.** Een python-heredoc verminkte voor de TWEEDE keer een `\n`-escape tot een echte regelovergang; gevonden met `node --check`. Terugkerende gereedschapsval: schrijf zulke patches als BESTAND weg in plaats van via een heredoc.
- **VOLGORDE-VOORSTEL, met grond.** Punt 17 en punt 11 vóór de TOOLING-stapel (22 t/m 25 en 31): 22 februari 2027 zet het doel op korte beklimmingen, en die twee punten hangen allebei aan dát doel. Punt 21 eerst TOETSEN op zijn nul-uitkomst — `zwoStepFromRow_` gaf over de hele populatie 0 keer `null`, dus de DSL-terugval is vermoedelijk dood — voordat er iets aan gebouwd wordt.

FOCUS VOLGENDE CHAT: ROADMAP punt 17 — de norm is voor de klim-doelen onbereikbaar. Verse chat.

**PUNT 12 IS AF EN STAAT LIVE — DE COACH ZEGT HET ALS JE DOEL NIET BINNEN JE UREN PAST (4 augustus 2026).** Op een blokgrens stelt hij FTP voor zodra het ingestelde doel meer weekuren vraagt dan er gedeclareerd staan; afwijsbaar, en het antwoord wordt bewaard. Drie commits: `ed86ef94b8597021e0b5d4cb34bc1849024c58b8` het bouwdoc, `6356221d6143eb4ece130ec8aa7b67cfd6cf486f` data plus worker, `293aa60db0b25e8bd60c2aa7793b6fed4efc7d0c` client plus harness. Migratie `0010_uneven_scarlet_spider.sql` — twee kolommen op `sync_state` — LOKAAL ÉN REMOTE toegepast. Worker Version `96c5a356-7d90-481a-b77a-c0fe91c8ac19` op <https://cadans-api.dtkorteweg.workers.dev>.
- **VLOEREN NU: vitest-totaal 942 over 73 bestanden · engine-selftest-assert-count 1648**, afgelezen uit de suite-uitvoer. De assert-count is onbewogen: deze bouw raakte `packages/engine` niet. Lees de vloeren uit de suite; hardcode ze nooit.
- **DE VLOEREN VAN DE KAART ZELF, herkomst PLAN.** Coach-canon uit `docs/DOELEN-SPEC.md`, geen geijkte drempel — er valt niets te bemonsteren, dus er hoort geen plateau-toets bij. Korte beklimmingen **5** uur (§3.3), Lange beklimmingen **6** (§3.4, de ONDERKANT van "zes à acht", want een kaart die vuurt is een ingreep), Conditie **4** (§3.5). FTP en Onderhoud dragen er GEEN: §3.1 noemt FTP bij weinig uren juist de ruggengraat en §3.2 geeft Onderhoud drie kwaliteitsdagen ook bij drie uur. STRIKT onder de vloer — bij precies 5 uur PAST Korte beklimmingen.
- **PROD-STAND EN WAT DAAN ZIET: NIETS, en dat is de bedoelde uitkomst.** Read-only gemeten op prod: `doel` is `FTP` en `week_uren` is `5`. FTP draagt geen vloer, dus de kaart is per constructie inert. LET OP VOOR FEBRUARI 2027: ook ná de overgang naar Korte beklimmingen blijft hij stil zolang er 5 uur gedeclareerd staat — 5 tegen vloer 5 past. Hij verschijnt pas bij Korte beklimmingen onder 5, Lange beklimmingen onder 6 of Conditie onder 4.
- **VERIFICATIE.** Zes rood-patches, elk LOS gedraaid en vooraf gegrept op de eigen markering, en elk raakte precies ÉÉN assertie. Het instrument is eerst geijkt met twee VOOR-runs: **93 van 93 identiek**. VOOR tegen NA: **85 identiek en 8 verschillend van 93 vergeleken**, en die acht zitten ALLEMAAL in het nieuwe scenario `doel-passend` — `01-week`, `02-ma`, `03-di`, `04-wo`, `05-do`, `06-vr`, `07-za` en `08-zo`. `v7/09-vorm` en `v7/10-trainingen` vielen buiten de vergelijking omdat ze niet byte-deterministisch zijn.
- **TWEE NEGATIEVE CONTROLES BEWOGEN NIET**, en dat is het bewijs dat de vloer de goede kant op staat: `klim-kort` en `klim-weekstem` dragen hetzelfde doel met `week_uren` 5 op blokweek 1, dus daar hoort de kaart NIET te verschijnen — en hij staat er op geen van beide.
- **HET BEELD LIET MEER ZIEN DAN DE TEST.** Op `doel-passend/01-week` verschijnt niet alleen de kaart; de dosis-trede-kaart is er VERDWENEN. Dat is `kaartPrecedentie` die zichtbaar doorwerkt tot op het scherm — één vraag tegelijk, en niet alleen in een unit-test.
- **DE JA-TIK VERZET OOK `doelStart`** naar de blokgrens-maandag, zodat het nieuwe doel met een eigen blok op Base begint. Dat gebeurt ALLEEN via deze kaart. BUITEN de kaart herstart een doelwissel de cyclus nog steeds niet — dat is ROADMAP punt 28, en het blijft open.
- **OPENSTAAND, NIEUW.** ROADMAP punt 29: de shot-harness controleert niet of de draaiende dev-worker bij de repo hoort. Punt 30: één uitvallende sub-request maakt het HELE weekscherm zwart in plaats van alleen de bijbehorende kaart weg te laten.
- **OPENSTAAND, ONGEWIJZIGD.** ROADMAP 11 (deels) · 13 · 16 · 17 · 19 · 21 · 22 · 23 · 24 · 25 · 28 · `indoor_ftp` 260 tegen `ftp` 280 · de 404 op `/api/checkin/<datum>` · de coach-copy-ronde · geen harness-scenario voor fase Peak.

VOORSTEL VOLGENDE CHAT, geen opdracht: ROADMAP punt 13 — na het event volgt geen herstel. Dat is het LAAGSTGENUMMERDE punt met status "open" nu 12 af is. LET OP BIJ HET KIEZEN: punt 11 (de duurvermogen-meetlat) staat op "deels" en is dus óók niet af; het is numeriek lager maar draagt een andere status. Punt 30 is nieuw en raakt de robuustheid van het weekscherm, wat elke volgende route-toevoeging duurder maakt — dat is een argument om het naar voren te halen. Verse chat.

**PUNT 10 IS GESLOTEN ZONDER BOUW EN PUNT 27 IS AF EN STAAT LIVE (4 augustus 2026).** Het aanbod "verschuif deze week de minuten naar Drempel" komt er NIET; de wat-als eronder haalde een live defect boven, en dát is gebouwd. Worker Version `76c80e90-cbf6-4587-804e-a97c5c8e6196`, was `01dee48d-f756-48bd-bec1-a25f0e5813a9`. 3 assets vervangen (`/index.html`, `/sw.js`, `/assets/index-Cyq4JBmp.js`), 63 ongewijzigd. GEEN migratie en GEEN ENKEL `wrangler d1`-commando, ook geen read; `0009_amusing_mordo.sql` blijft de hoogste. Commits: verdict `7a5a278` (`docs/PUNT10-FASE-B-DEEL2-VERDICT.md`), bouw `b57d464`, plus deze close-out. CI success op de bouw, run <https://github.com/daanhhk/Cadans/actions/runs/30888009985>.
- **VLOEREN NU: vitest-totaal 923 over 71 bestanden · engine-selftest-assert-count 1648**, afgelezen uit de suite-uitvoer. De assert-count is ONBEWOGEN omdat dit CLIENT-only is; `git diff --stat` over `packages/engine` én `workers/api` is leeg. Lees de vloeren uit de suite; hardcode ze nooit.
- **DE GROND VAN HET VERDICT.** GEMETEN over 630 cellen — 7 weekvormen maal 5 doelen maal 3 fase-ankers maal 6 dagoffsets. De weekstem vuurt in 119 cellen, en die splitsen zonder rest: in **75** is er geen trainingsdag meer over, dus er valt per constructie niets te verschuiven, en in de andere **44** schrijft de restdag de gemelde tekortzone AL voor — 18,8 tot 32,4 drempel-minuten met het nominale label `drempel`. Het aanbod zou dus drempelwerk vervangen door drempelwerk: NUL cellen waarin het iets toevoegt. Zelfde vorm als punt 5c, en met dezelfde grond: niet "de tak is onbereikbaar" maar "de uitkomst is geen verbetering".
- **DE BLINDE VLEK DIE DAT OPLEVERDE, en die was live.** In die 44 van de 119 vuur-cellen was de zin "er staat geen trainingsdag meer om die prikkel op te pakken" ONWAAR. En het sneed beide kanten op: een GEMISTE efforts-rit telde in 0 van 28 cellen als open sleutelprikkel, en een te LICHT GEREDEN in 0 van 28. Met de term zwijgt de stem in exact die 44, en gaan alle vijf de doelen naar 15 vuur-cellen — waar het nu Conditie 15, FTP 15, Onderhoud 15, Korte beklimmingen 37 en Lange beklimmingen 37 is. De 22 extra per klim-doel waren dus volledig artefact.
- **HET VINDPATROON GAF ÉÉN TREFFER.** Van de zes types die de planner over de meetruimte oplevert draagt alleen `combo_long_with_efforts` een werkzone zónder sleutelsessie te heten. `combo_all_three` is een latent tweede geval maar heeft nul producenten (punt 14 fase 2).
- **HET BEELD.** Buiten het nieuwe scenario **77 shots identiek**, en alle 8 shots van `klim-weekstem` bewegen. VOOR stond op vrijdag het blok `DEZE WEEK · PRIKKEL WEG` met "er komt geen trainingsdag meer om hem in te halen"; NA zijn die regels weg, en op de gemiste maandag staat `WAAR DE PRIKKEL NU STAAT` met `za 8`, `Lange rit + Korte beklimmingen efforts (120 min)`, 120 min.
- **NIEUW SCENARIO `klim-weekstem`** in `tools/shots/shot.mjs`: het EERSTE dat een klim-doel met een dagOffset combineert. blokWeek 5, dus Build — `combo_long_with_efforts` vuurt alleen in Build en Peak, dus blokweek 1 kon dit geval per constructie niet tonen.
- **WAT DAAN MERKT: NIETS, nu.** Op prod staat doel `FTP / drempel`, en `spreiding.effortsInLangeRit` staat op `true` bij precies twee profielen (`archetypes.ts:1622` en `:1664`) en op `false` bij de andere drie. De bouw is daar dus per constructie inert, en de prod-shots bevestigen dat: het blok `PRIKKEL WEG` komt op geen enkele van de 15 shots voor. Het wordt dragend zodra het doel half februari 2027 naar Korte beklimmingen gaat.
- **DE DRIE AFWIJKINGEN UIT DE BOUWRONDE, kort.** `pDagen` in `weektekort.ts` is NIET verplaatst en hoefde niet, want `blokkenVoor_` doet zijn eigen lookup. Een regex-substitutie in `sleutelinhaal.test.ts` brak twee multi-line aanroepen; met de hand hersteld. En de vite-dev-server viel twee keer om, waarna de voor- en na-run opnieuw en achter elkaar op dezelfde machine zijn gedraaid.
- **HET ONTBREKEN VAN EEN PROD-VOOR-STAAT IS GEEN INCIDENT MAAR EEN INSTRUMENT-DEFECT.** De harness heeft ÉÉN uitvoerpad voor een lokale én een prod-run (`shot.mjs:19`) en leegt dat pad bij ELKE run (`:758`); de prod-vlag op `:30` kiest er geen ander pad bij. Draai je lokaal na een prod-run, dan is de prod-uitvoer weg. NU TWEE KEER GERAAKT — punt 26 en punt 27 — en beide keren kon daardoor de sterkste bewijsvorm, een byte-vergelijking voor en na op prod, niet gedraaid worden. Ik meldde het beide keren als afwijking; dat was juist, maar de oorzaak lag niet bij de ronde. Staat als TOOLING-item op de parkeerlijst, met de fix erbij: een eigen pad voor de prod-run.
- **OPENSTAAND, ONGEWIJZIGD.** ROADMAP 12 · 13 · 16 · 17 · 19 · 21 · 22 · 23 · 24 · 25 · `indoor_ftp` 260 tegen `ftp` 280 · de 404 op `/api/checkin/<datum>` · de coach-copy-ronde · geen harness-scenario voor fase Peak.

FOCUS VOLGENDE CHAT: ROADMAP punt 12 — doel-passendheid. Het eerstvolgende open punt in de volgorde, en CLIENT. De coach stelt een passend doel voor als het ingestelde doel niet binnen de uren past: afwijsbaar, en hoogstens één keer per blok op een blokgrens. De norm staat in `DOELEN-SPEC` §2A onder DOEL-PASSENDHEID en in §6 stap 6. Hangt aan punt 4 en punt 7, allebei af. Verse chat.
