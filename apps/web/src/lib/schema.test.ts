...
describe("deriveSchemaView dispatch (flip + doneCompare)", () => {
  ...
  // ── B4 make-up-post-pass (laag-2a) ── threshold (→ intent 'drempel' = KEY) → coach.adapt truthy.
  const MK_SETTINGS: SettingsInput = {
    ftp: 250,
    lthr: 160,
    gewicht: 72,
    doel: null,
  ...
  // Gemist bron-dag (threshold, disposition, geen rit) + latere plannbare (rust)dag als target.
  const mkWeek = () =>
    pweek([
      pday("2026-03-10", {
        dagIdx: 0,
      ...
      pday("2026-03-12", { dagIdx: 2 }),
    ]);

  it("make-up GEMIST + target: bron-dag krijgt de coachAdaptatie_-payload op de eerstvolgende plannbare dag", () => {
    const v = deriveSchemaView(
      mkWeek(),
      {},
      TODAY,
      { "2026-03-10": "geen_tijd" },
      [],
      null,
      MK_SETTINGS,
    );
    const bron = v.days.find((d) => d.datum === "2026-03-10");
    expect(bron?.state).toBe("gemist");
    expect(bron?.coach?.state).toBe("missed");
    const mk = bron?.makeupAdaptatie as unknown as {
      dISO: string;
      type: string;
      from: string;
      durMin: number;
    ...
    expect(mk.dISO).toBe("2026-03-12"); // eerstvolgende plannbare dag
    expect(mk.type).toBe("library");
    expect(mk.from).toBe("2026-03-10"); // bron-tag (idempotentie)
    expect(mk.durMin).toBeGreaterThanOrEqual(45);
    expect(mk.durMin).toBeLessThanOrEqual(plannedSS.totaalMin);
  });

  it("make-up IDEMPOTENT: bestaande override met from=bron → {planned:true} (geen nieuwe adaptatie)", () => {
    const v = deriveSchemaView(
      mkWeek(),
      {},
      TODAY,
    ...
    const bron = v.days.find((d) => d.datum === "2026-03-10");
    expect(bron?.makeupAdaptatie).toEqual({
      planned: true,
      dISO: "2026-03-12",
    });
  });

  it("make-up GEEN TARGET: enige latere dag heeft al een override → makeupAdaptatie null", () => {
    const v = deriveSchemaView(
      mkWeek(),
      {},
      TODAY,
    ...
    const bron = v.days.find((d) => d.datum === "2026-03-10");
    expect(bron?.state).toBe("gemist");
    expect(bron?.makeupAdaptatie).toBeNull();
  });

  it("make-up CLAIM: twee gemiste bron-dagen + één target → eerste claimt, tweede krijgt null", () => {
    const v = deriveSchemaView(
      pweek([
        pday("2026-03-09", {
          dagIdx: 0,
        ...
        pday("2026-03-12", { dagIdx: 3 }),
      ]),
      {},
      TODAY,
      { "2026-03-09": "geen_tijd", "2026-03-10": "geen_tijd" },
      [],
      null,
      MK_SETTINGS,
    );
    const b1 = v.days.find((d) => d.datum === "2026-03-09");
    const b2 = v.days.find((d) => d.datum === "2026-03-10");
    const mk1 = b1?.makeupAdaptatie as unknown as { dISO: string };
    expect(mk1.dISO).toBe("2026-03-12"); // eerste bron claimt de enige target
    expect(b2?.makeupAdaptatie).toBeNull(); // tweede: target al geclaimd → null
  });
});
