...
CHECK 42
CONDITIE : ALTIJD
TOETS    : ga je AFWEZIGHEID vaststellen — "X staat er niet tussen", "het veld is er niet", "die scope ontbreekt" — toon dan eerst dat je het VOLLEDIGE beeld hebt. Twee vragen, allebei stellen: (1) is de uitvoer waar ik naar kijk AFGEKAPT (`head`, `tail`, een `| head -20`, een tabel die afkapt, een truncated tool-resultaat)? (2) kan mijn ZOEKPATROON het gezochte per constructie missen — staan er cijfers, punten, koppeltekens of hoofdletters in de naam die mijn karakterklasse niet toelaat? TEL het totaal en vergelijk dat met wat je ziet.
UITKOMST : ofwel je hebt het volledige beeld en de afwezigheid is een MEETRESULTAAT, ofwel je stelt vast dat je het niet kunt zien en dan is het "NIET GEMETEN" en geen "afwezig". GEMETEN 24/25-08-2026: ik concludeerde dat een wrangler-token de `d1`-scope miste, en dat werd ROADMAP punt 76, een deel van een rapport en een blokkade op een migratie. Het token DROEG `d1:write`. Twee keer dezelfde fout in een andere vorm: eerst las ik de scopes uit een met `head`/`tail` afgekapte `whoami`-uitvoer, daarna uit het bestand met het patroon `"[a-z_:]+"` — dat geen CIJFERS toelaat, en `d1:write` draagt er een. Het volledige beeld telde 28 scopes waar ik er 23 zag.
HERKOMST : de diagnose-ronde van 25-08-2026; de aanleiding is een onjuist ROADMAP-punt dat een dag lang als feit rondging.

CHECK 43
CONDITIE : ALTIJD
TOETS    : schrijf je een string die de GEBRUIKER een oordeel laat vellen — "jij weet of ...", "klopt dit?", "neem over of laat staan" — loop dan af WELKE GROOTHEDEN hij nodig heeft om dat oordeel te kunnen vellen, en controleer of ze alle in de zin staan. Kijk in het bijzonder naar aanwijzende woorden ("die dag", "deze rit", "dat blok"): elk aanwijzend woord moet iets aanwijzen dat de zin zelf noemt. En loop de SELECTIE na die het getoonde geval koos — zegt de zin iets over recentheid, volgorde of uniciteit dat die selectie niet waarmaakt?
UITKOMST : ofwel de zin draagt alles wat het oordeel vraagt, ofwel je hebt een rem ingebouwd die niet te bedienen is. GEMETEN 25-08-2026 in punt 69: het FTP-voorstel schrapte bewust zijn plausibiliteitsgrens en zette de renner ervoor in de plaats (M10) — hij is daarmee de ENIGE rem die het ontwerp nog heeft. De kaart noemde naam, duur, vermogen en factor, maar NIET de datum, en sloot af met "Jij weet of je die dag echt diep ging" terwijl "die dag" nergens in de zin stond. Erger: bij een lege ritnaam viel hij terug op "je laatste rit", en dat is een RECENTHEIDSCLAIM die de selectie niet waarmaakt — de poort kiest de HOOGSTE piek uit alle openstaande ritten, met de datum alleen als scheidsrechter bij gelijke pieken, dus de getoonde rit kan dagen ouder zijn dan de rit van vanochtend. De renner zou een drempelwaarde bevestigen op een rit die hij verkeerd identificeerde. DIT IS DE COPY-KANT VAN M55: de zin loog niet over wat de app DEED, maar liet weg wat de LEZER nodig had, en dat is dezelfde schade.
HERKOMST : punt 69, 25-08-2026 — gevonden door de tweede weerleggingspas op de gebouwde code, door drie lenzen onafhankelijk, daarna zelf nagemeten.
