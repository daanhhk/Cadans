...
**T28 FASE 3 DEELS — de pendel-WEERGAVE-lijn is compleet (juli 2026).** HEAD `9a0a5d0`, CI success.
- **VLOEREN NU: vitest-totaal 451 · engine-selftest-assert-count 997.** Lees ze uit de suite; niet hardcoden in prompts.
- **3a (`aaab804`) — `pendelDuurMin` = duur PER RIT.** De settings-UI verdubbelde de invoer (`legToRoundTrip`); dat was de enige echte bug (de engine leest de waarde ongehalveerd en × `pendelAantal`, byte-identiek aan GAS). Verwijderd. **GEDEPLOYED.**
- **3b (`72ce7eb`) — binnen-sessie Heen/Terug-split weg.** `genericPendelZ2` → één blok "Hele rit"; `genericPendelIntervals` → "Warming-up" + het werkblok. Belasting byte-identiek (totaalMin/tss/zones/structuur). **GEDEPLOYED.**
- **3b-copy (`bc95df1`) — richting-bewuste notes.** Rit 1 = heen (rustig), rit 2 = terug (prikkel). Optionele `leg`-param op `genericPendelZ2` + `buildWorkout` (default "heen" → byte-identiek); de warmup-note "Aanrijden naar werk" → "Rustig op gang" omdat die generic ALTIJD de terugrit is. **NOG NIET GEDEPLOYED.**
- **DEPLOY-STAND.** Prod Worker Version `d53c193d` = fase 1 + 2a + 3a + 3b. Migratie `0005` toegepast op REMOTE D1 (de `week_uren`-kolom bestaat). 3b-copy (`bc95df1`) wacht op de volgende uitrol. Pendel-waarde in remote D1 (user 1): **`pendel_duur_min = 75`, `pendel_aantal = 2`** — Daan heeft in de UI herbevestigd, dus de per-rit-semantiek klopt nu.
- **DEPLOY-PROCEDURE-CORRECTIE.** `npx wrangler deploy` MOET vanuit `workers/api` draaien (de gepinde 4.106.0, waar `wrangler.jsonc` staat) — NIET vanuit de repo-root, die pakt een nieuwere wrangler en faalt op "workspace root". De migratie-volgorde blijft: remote-D1-migratie EERST (vanuit `workers/api`), DAARNA de deploy.
...
1. **[HOOG] Dinsdag-training ongevraagd "ingekort" bij readiness 95 ("Klaar om te trainen").** Verdacht: bij readiness *ready* hoort het week-signaal `normal` te zijn — geen demote. Daan meldt dat de inkorting er al vóór de pendel-config stond en er losstaat → hypothese: een persistente day-override (D1 `day_state`/overrides) die blijft plakken, óf een bug in de opt-in verlicht-/band-demote-laag (fase 2a / laag 2). Recon: lees de inkort-conditie, check of er een override op die datum in remote D1 staat, en de weekband-stand. **Verifieer eerst of de inkorting in de VERSE bundle nog bestaat** (Daan-screenshot van DI in de lopende week).
2. **[MID] Vermogenszones ogen te hoog / bron onduidelijk.** Pendel-Z2 168-202W vs lange-Z2 216-227W — twee verschillende Z2-ranges, de tweede boven de warmup-top (140-190W). Wijst op een zone-bron-mismatch. Recon: waar komen de zones/FTP vandaan (Intervals vs lokaal berekend) en waarom verschilt Z2 per workout-type.
3. **[LAAG] Dagteller toont 1/4 bij 5 trainingsdagen.** De teller "DAGEN" telt een dubbele training niet dubbel, maar 4 ≠ 5 → mogelijk een noemer-bug of config-restje. Check of het na de goede pendel-config nog bestaat.
4. **[NIEUWBOUW] 3c — per-rit gepland-vs-gedaan-koppeling** (§2 van `docs/T28-FASE3B-PENDEL-RECON.md`). GAS koppelt óók per dag → dit is een Cadans-eigen verbetering, geen parity. Open ontwerpvraag: de koppelregel (tijdstip/volgorde/duur) + de half-gereden-status (§5 van dat recon-doc). Nog niet aangeraakt.
5. **[NIEUWBOUW] 3d — effectief weekdoel + weekfeedback.** Het globale urengetal als duur-consument (± ~1u fase-modulatie, mét uitleg) + weekfeedback op de zondag-invoer. Recon: `docs/T28-FASE3-RECON.md`. Nog niet begonnen.
- Ook geparkeerd (losse tune): de warmup-verhouding in `genericPendelIntervals` — warmup = `floor(mins/2)` ≈ 37 min bij 75.
- **FOCUS VOLGENDE CHAT: werkstroom 1 (de ongevraagde inkorting)** — het enige HOOG-item. Recon-first; verse chat.
